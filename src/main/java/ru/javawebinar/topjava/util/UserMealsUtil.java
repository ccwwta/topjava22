...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        meals.forEach(m -> sums.merge(m.getDateTime().toLocalDate(), m.getCalories(), Integer::sum));

        List<UserMealWithExcess> filtered = new ArrayList<>();
        meals.forEach(m -> {
            if (TimeUtil.isBetweenHalfOpen(m.getDateTime().toLocalTime(), startTime, endTime)) {
                filtered.add(
                        new UserMealWithExcess(m.getDateTime(), m.getDescription(), m.getCalories(),
                                sums.get(m.getDateTime().toLocalDate()) > caloriesPerDay)
                );
            }
        });
